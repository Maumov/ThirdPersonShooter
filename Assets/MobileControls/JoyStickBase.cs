...
public class JoyStickBase : MonoBehaviour {
	...
	static private JoyStickBase[] joysticks ;					// A static collection of all joysticks
	...
	void Update () {
		...
		if ( !enumeratedJoysticks )
		{
			...
			joysticks = FindObjectsOfType<JoyStickBase>() as JoyStickBase[];
		...
		else
		{
			for(int i = 0;i < count; i++)
			{
				...
				if ( shouldLatchFinger && ( lastFingerId == -1 || lastFingerId != touch.fingerId ) )
				{
					...
					foreach ( JoyStickBase j  in joysticks )
					{
						if ( j != this )
